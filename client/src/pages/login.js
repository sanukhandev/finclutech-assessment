...
const LoginPage = () => {
    ...
    const handleFormSubmit = async(event) => {
        ...
        const loginPromise =  await fetchHookWithBody('users/login', {method: 'POST'}, data);
        if(loginPromise.status === 'ERROR') {
            ...
            const {token,...user} = await loginPromise.data
            setIsAuth(true)
            dispatch({
                type: "LOGIN",
                payload: {
            ...
            <div className="flex justify-center items-center mt-6">
                <p className=" inline-flex items-center text-gray-700 font-medium text-xs text-center ">
                    <span className="ml-2">You don't have an account?
                        <a href="/register" className="text-xs ml-2 text-blue-500 font-semibold" >Register now</a></span>
                </p>
            </div>
        </div>
    )
